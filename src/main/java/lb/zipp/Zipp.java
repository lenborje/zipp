...
public class Zipp implements Closeable {
	private static final ResourceBundle cMessages = ResourceBundle.getBundle(Messages.class.getName());
	...
	protected static final String getMessage(final String key) {
		return cMessages.getString(key);
	}
	 ...
	 */
	public Zipp(final String archiveName, Option... options) throws IOException, URISyntaxException {
		this.options = Collections.unmodifiableSet(Stream.of(options).collect(toSet()));
		this.recursive = this.options.contains(Option.RECURSIVE);
		this.parallel = this.options.contains(Option.PARALLEL);
		...
		System.out.printf(getMessage("working"), zipURI, this.options);
	...
	public void zipOneFile(final Path f) {
		try {
			...
			String message = " "+getMessage("adding")+": %s";
			if (Files.exists(zipEntryPath)) {
				Files.deleteIfExists(zipEntryPath);
				message = " "+getMessage("updating")+": %s";
			}
			...
			final String methodName = method==0?"stored":method<8?getMessage("compressed"):getMessage("deflated");
	...
	public void addFiles(final List<String> fileNameArgs) {
		...
		final Stream<Path> distinctPaths =
			...
			.distinct();					// NB! Necessary!

		// Do NOT remove the distinct() call! 
		// Doing so can defeat the desired parallelism, because the
		// filesWalk stream does not parallelise cleanly.
		// Adding distinct(), which is an intermediate operation,
		// will cause all directory traversals to be resolved, 
		// and ensures all files will be processed in parallel in the next step.
		// (This is because distinct() creates a Collection of the files paths,
		// which parallelises very well.)
	...
	private Stream<Path> filesWalk(final File f) {
		...
		try {
			...
			throw new RuntimeException(String.format(getMessage("errtrav"), f), e);
		}
	...
	private static void usage() {
		System.err.println();
		System.err.printf(getMessage("usage1"));
		System.err.println();
		System.err.println("");
	}
	...
	public static void main(final String[] args) {
		try {
			...
			if (fileArgs.size()<2) {
				throw new NotEnoughArgumentsException(getMessage("noargs"));
			}
			...
			try (Zipp zip = new Zipp(zipName, options)) {  // Initialise zip archive
				...
				System.out.println(getMessage("zpclos"));
			} 
			
			System.out.println(getMessage("zpdone"));
